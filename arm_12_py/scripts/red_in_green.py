...
try:
    trans = tf_buffer.lookup_transform("rotated_base", "straw2", rospy.Time(0), rospy.Duration(1.0))
...
def main(mode="cartesian"):  # mode: "joint" or "cartesian"
    ...
    # T_2[:3,:3] = rotation_matrix("y",-0)?
    T_2[:3,:3] = rotation_matrix("y",-90)
    T_2[:3,3] = np.array([0,0.05,-0.09])
    T_new = T_new.dot(T_2)
